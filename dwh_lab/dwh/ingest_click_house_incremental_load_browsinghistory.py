...
def main(entity_name):
    ...
    # ✅ QUALITY REPORT
    qr = QualityReport(df, entity_name="browsinghistory")
    qr.summarize_volume()
    qr.check_nulls(required_columns=["entry_id", "exit_page", "pageview_count","referrer_page","search_keyword","timestamp","title","tmp_keywords","url","user_id","visible_content"])
    qr.check_duplicates(dedup_columns=["entry_id", "exit_page", "pageview_count","referrer_page","search_keyword","timestamp","title","tmp_keywords","url","user_id","visible_content"])

    report = pd.json_normalize(qr.generate()).to_dict(orient="records")[0]

    JOB_DATE = datetime.now().strftime('%Y%m%d')
    os.makedirs("reports", exist_ok=True)
    with open(f"reports/quality_report_clickhouse_browsinghistory/quality_report_clickhouse_incremental_{entity_name}_{JOB_DATE}.json", "w", encoding="utf-8") as f:
