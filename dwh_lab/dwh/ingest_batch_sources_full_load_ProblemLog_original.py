...
def main(entity_path):
    ...
    if 'junyi_ProblemLog_original' in entity_name:
    ...
    entity_name = entity_path.split("/")[-1].split(".")[0]
    if 'junyi_ProblemLog_original' in entity_name:
        entity_name = 'junyi_ProblemLog_original'
    ...
    qr.summarize_volume()
    qr.check_nulls(required_columns=['user_id', 'exercise', 'problem_type', 'problem_number', 'topic_mode',
        'time_done', 'time_taken', 'correct', 'count_attempts', 'points_earned'])  # hoặc cột tùy theo entity
    qr.check_duplicates(dedup_columns=['user_id', 'exercise', 'problem_type', 'problem_number', 'time_done'])  # hoặc các cột phù hợp
    qr.check_array_fields(array_fields=['hint_time_taken_list'])
    ...
    with open(f"reports/quality_report_ingest_batch_ProblemLog_original/quality_report_ingest_batch_full{entity_name}_{JOB_DATE}.json", "w", encoding="utf-8") as f:
